...
def find_haveproduct_stations(Single_robot):  # 如果机器人没有携带产品，查找机器人可以购买产品的工作台编号
    ...
    if Single_robot.carried_item_type == 0:
        for j in range(len(workstations)):
            ...
            if workstations_lock[j][0] != 0 or isfreeworkstation[0][j] == 0:
                continue
            if ((workstations[j].product_status == 1) or (
                    workstations[j].proc_time <= dis / 2 * 50 and workstations[j].proc_time >= 0)):
    ...
    for i in range(len(workstations)):  # 查找编号为i的工作台可以将产品卖到那些工作台
        if bots_to_product[0][i] == 1:
            ...
            if work_type == 1:  # 如果是1号产品，则型号为4,5,9的工作台可以收购该产品
                for j in range(len(workstations)):
                    if workstations_lock[j][1] != 0 or isfreeworkstation[0][j] == 0:  # 如果j号工作台已经成为另一个机器人的目标，则不能成为目标
                        continue
            ...
            if work_type == 2:
                for j in range(len(workstations)):
                    if workstations_lock[j][2] != 0 or isfreeworkstation[0][j] == 0:
                        continue
            ...
            if work_type == 3:
                for j in range(len(workstations)):
                    if workstations_lock[j][3] != 0 or isfreeworkstation[0][j] == 0:
                        continue
            ...
            if work_type == 4:
                for j in range(len(workstations)):
                    if workstations_lock[j][4] != 0 or isfreeworkstation[0][j] == 0:
                        continue
            ...
            if work_type == 5:
                for j in range(len(workstations)):
                    if workstations_lock[j][5] != 0 or isfreeworkstation[0][j] == 0:
                        continue
            ...
            if work_type == 6:
                for j in range(len(workstations)):
                    if workstations_lock[j][6] != 0 or isfreeworkstation[0][j] == 0:
                        continue
...
def bots_coordinate_motion(best_path, bot_status, bot_control,map_all_workstation_path, start_paths):
    for i in range(len(bots)):  # 分别控制每一个机器人运动
        ...
        if rob_path_information[i][3] != -100 and rob_path_information[i][4] != -100:
            ...
            bot_status,bot_control = control_to_goal(bot_control , bots, best_path, bot_status, i)  # 加返回值
...
if __name__ == '__main__':
    ...
    while True:
        ...
        map_all_workstation_path, start_paths = bots_coordinate_motion(best_path, bot_status, bot_control, map_all_workstation_path,
                                                                       start_paths)  # 控制多个机器人运动，包括目标选取、路径选择
